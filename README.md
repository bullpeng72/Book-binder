...
- **fix**: PDF 변환 시 세로형 Mermaid 다이어그램(순서도 등)이 여러 페이지에
  걸칠 때, 스크린샷 청크 크기가 실제 페이지 크기와 무관한 고정값(300px)이라
  페이지 하단에 큰 공백이 남던 문제 수정 — 청크 목표 크기를 실제 PDF 페이지
  콘텐츠 높이 기준으로 계산해, 다이어그램이 매 페이지를 거의 끝까지 채우도록
  개선했다.
- **fix**: 위 계산이 다이어그램이 시작하는 지점의 "페이지에 남은 공간"을
  문서 흐름 높이 기준 나눗셈(y % 페이지높이)만으로 추정해, 제목·표·인용구
  등의 페이지 나눔 회피(`break-inside`/`break-after: avoid-page`) 규칙이
  실제 페이지 경계를 앞당기는 걸 반영하지 못하던 문제 수정 — 챕터 뒤쪽으로
  갈수록 오차가 누적돼, 소제목 하나만 덩그러니 남고 다이어그램 전체가 다음
  페이지로 밀려 이전 페이지가 거의 텅 비는 사례가 있었다. 각 다이어그램
  위치에 임시로 고정 높이 줄로 된 "자"를 넣어 실제로 한 번 인쇄해 보고, 그
  자가 어느 페이지에서 잘리는지 읽어 진짜 남은 공간을 측정하는 방식으로
  교체했다.
