...
- **fix**: macOS(NFD)로 저장된 파일명과 `book.yaml`(보통 NFC)의 정규화 형태가
  달라 `exclude`/`section_id_overrides`가 시각적으로 동일한 파일명을 놓치던
  문제 수정 — 비교 전 양쪽을 NFC로 통일.
- **fix**: 웹 편집기 `/api/images/serve`가 문자열 접두사 비교로 경로를
  검증해 형제 디렉토리(예: `book_evil`)로 화이트리스트를 우회당할 수 있던
  문제 수정 — `Path.is_relative_to()`로 교체.
- **feat**: 병합 PDF(`--merge`)에 챕터별 북마크(아웃라인) 추가 — Part가
  있는 코퍼스는 Part 제목 아래 챕터가 중첩된 구조로, PDF 뷰어 사이드바에서
  바로 챕터로 이동할 수 있다.
- CLI `--help` 텍스트 현행화 — 옵션별 기본값 결정 규칙, 필요한 extra 설치
  안내, 실행 예시를 각 명령에 추가.
- 회귀 테스트 33건 추가(총 70개).
